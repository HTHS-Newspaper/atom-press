...
const categories = [
	{ name: "Home", href: "/" },
	{ name: "@High Tech", href: "/high-tech" },
	{ name: "Entertainment", href: "/entertainment" },
	{ name: "U.S. and World News", href: "/news" },
...
const links = [
	...
	{ name: "Contribute", href: "/dashboard" },
];
	...
	return (
      ...
      ${
		...
		>
			{" "}
			<div
				className={`md:h-viewscreen relative pl-4 pb-4 md:pb-0 w-full max-w-2xl h-full ${quicksand.className} md:bg-default md:drop-shadow-lg text-textpink  overflow-auto`}
			>
				<div className="flex items-start justify-between p-4 text-3xl place-content-center">
					...
					<button onClick={() => props.closeBar(false)}>
						<svg
							...
							className="duration-150 ease-in-out hover:scale-105"
						>
							<path
					...
					<Link
						...
						className={`flex mt-2 rounded-l-lg justify-between place-content-center items-start p-4 text-2xl hover:bg-defaultDark  hover:text-default  duration-150 ${
							item.name === props.page
								? "headerbold bg-defaultDark text-default"
								: ""
						}`}
					>
						{item.name}
				...
				<div className="my-4 mr-4 border border-b-4 rounded-full border-defaultDark bg-defaultDark"></div>

				{links.map((item) => (
					<Link
						key={item.name}
						href={item.href}
						className="flex items-start justify-between p-4 text-2xl duration-150 rounded-l-lg place-content-center hover:bg-defaultDark hover:text-default"
					>
						{item.name}
					</Link>
				))}
